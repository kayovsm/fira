...
void acelera(double vel_esquerda, double vel_direita)
{
...
void ajuste(double referencia, double valor_acelera)
{
  ...
  // double valor_esquerda, valor_direita;

  // valor_esquerda = OutputE; 
  // valor_direita = OutputD;

  // acelera(valor_esquerda,valor_direita);

  if (referencia > 0)
  {
    acelera(0, MIN_VOLTAGE);
  }
  // mais a esquerda
  else if (referencia < 0)
  {
    acelera(MIN_VOLTAGE, 0);
  }
  else
  {
...
void loop()
{
  ...
  while (true)
  {
    ...
    distanciaD_abs = abs(distanciaD - DIS_MAX);
    distanciaE_abs = abs(distanciaE - DIS_MAX);
